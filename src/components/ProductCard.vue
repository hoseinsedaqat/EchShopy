<template>
  <section class="product-card">
    <div class="p-col-1">
      <img src="../assets/images/card/supermarket.png" alt="" />
    </div>
    <div class="p-col-2">
      <swiper class="swiper" :options="swiperOption">
        <swiper-slide v-for="(mrt, idx) in MarketAmazing" :key="(mrt, idx)">
          <div class="card-col-1">
            ...
            <div class="p-offer">
              <del>{{ Number(mrt.realPrice).toLocaleString() }}</del>
              <span>{{ mrt.precent }}</span>
            </div>
            <p class="p-price">{{ Number(mrt.finalOffer).toLocaleString() }} تومان</p>
...
export default {
  ...
  methods: {
    ...
    aTC(idx) {
      ...
      const aProduct = this.MarketAmazing[idx];
