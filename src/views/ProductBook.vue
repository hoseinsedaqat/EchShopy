        ...
        <div class="kala-last-col">
          ...
          <div class="kala-price">
            ...
            <p class="kala-price-2">
              {{ Number(getBook.finalOffer).toLocaleString() }} تومان
            </p>
          </div>
...
export default {
  ...
  methods: {
    ...
    aTC() {
      ...
      this.addToCart(this.getBook);
    },
    showImgGalley() {
      this.$swal({
        confirmButtonText: "بستن",
        showCloseButton: false,
        imageUrl: this.getBook.img,
      });
    },
  },
  computed: {
  ...
  mounted() {
    if (!this.BookCol[this.$route.params.id - 17]) {
      this.$router.push({ name: "ErrorPage" });
    }
    // use 1 because in firefox 0 dose not work !!
    document.body.scrollTop = 1;
    document.documentElement.scrollTop = 1;
    this.getBook = this.BookCol[this.$route.params.id - 17];
  },
};
</script>
